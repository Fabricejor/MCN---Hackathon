...
> = {
  ...
  amber: {
    frame: 'bg-gradient-to-b from-[#D4AF37] to-[#c8a231]',
    accent: 'text-[var(--brown)]',
    btn: 'bg-[var(--gold)] text-[var(--black)]',
    btnHover: 'hover:brightness-95',
    okBg: 'bg-[var(--beige)]',
    okRing: 'ring-[var(--gold)]/30',
    okFg: 'text-[var(--brown)]',
    noBg: 'bg-red-50',
    noRing: 'ring-red-200',
    noFg: 'text-red-500',
  ...
  return (
    <section aria-label={`${name} plan`} className={cn('relative', className)}>
      ...
      <div className={cn('rounded-3xl p-[3px]', t.frame)}>
        {/* card surface */}
        <div className='rounded-[22px] bg-[var(--light)] px-8 pb-8 pt-10 shadow-sm ring-1 ring-[var(--gold)]/20'>
          {/* icon */}
          {icon && (
            <div
              className={cn(
          ...
          <h3
            className={cn(
              'text-center text-xl font-semibold font-serif text-[var(--brown)]',
              nameClassName
            )}
          >
            {name}
          ...
          {subtitle && (
            <p
              className={cn(
                'mt-1 text-center text-sm text-[color:rgb(0_0_0_/_0.65)]',
                subtitleClassName
              )}
            >
              {subtitle}
          ...
          {/* price */}
          <div className={cn('mt-6 text-center font-serif', t.accent, priceClassName)}>
            <span className='text-5xl font-bold leading-none'>{pp.main}</span>
            <span
              className={cn(
                'ml-1 text-[color:rgb(0_0_0_/_0.65)]',
                periodClassName
              )}
            >
              {periodLabel}
            ...
            {features.map((f, i) => {
              ...
              return (
                <li
                  key={i}
                  className={cn(
                    'flex items-start gap-3 text-[var(--black)]',
                    featureItemClassName
                  )}
                >
                  <span
